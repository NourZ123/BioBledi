    ...
    <div class="carte-presentation">
      ...
      <div class="liste">
        <div class="entete-carte">
          <img src="../images/leaves-svgrepo-com.svg" alt="leaves" class="icone" />
          <div>
            <h3>Pourquoi choisir BioBladi ?</h3>
          </div>
        </div>
        <ul>
      ...
      <div class="formulaire-recherche" style="max-width: 550px; margin: 0 auto;">
        ...
        <form method="GET" action="about us.php#resultats">
          <div class="form-row">
            <div class="form-group">
              ...
              <select name="recherche-critere" id="recherche-critere" class="form-select">
                // pour que le critère sélectionné reste sélectionné après la soumission du formulaire, on vérifie si
                $critere correspond à la valeur de chaque option et si oui, on ajoute l'attribut "selected" à cette
                option
                <option value="nom_produit" <?php if ($critere == 'nom_produit')
                  echo 'selected'; ?>>Par nom</option>
                <option value="région" <?php if ($critere == 'région')
                  echo 'selected'; ?>>Par région</option>
              </select>
            </div>
            <div class="form-group">
              <label for="recherche-valeur">Valeur :</label>
              <input type="text" name="recherche-valeur" id="recherche-valeur" class="form-input" placeholder="Ex: Sfax"
          ...
          <div class="btn-group-recherche">
            ...
            <a href="about us.php#resultats" class="btn-tous"
              style="text-decoration:none;line-height: 1.1; display:flex; align-items:center; justify-content:center; background-color: #fff; border: 2px solid #2e7d32; color: #2e7d32; font-weight: bold; flex: 1; border-radius: 40px; height: 45px;font-size: 0.85rem;">
              📋 Nos promos
            </a>
          </div>
        </form>
            ...
            foreach ($produits as $p): ?>
              <tr class="ligne-produit">
                ...
                <?php if (!isset($displayed_regions[$p['région']])): ?>
                  <td rowspan="<?php echo $regions_counts[$p['région']]; ?>"
                ...
                <td>
                  <?php echo htmlspecialchars($p['catégorie']); ?>
                </td>
                <td>
                  <?php echo htmlspecialchars($p['unité']); ?>
                </td>
                <td>
